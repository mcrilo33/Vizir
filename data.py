...
class MongoManager():
    ...
    def get_running_rows(self, db_name, columns):
    ...
    def get_exp_infos(self, db_name, id):
        collection = self.init_connection_to_runs(db_name)
    ...
    def get_rows_from_exp_names(
        ...
        filter_date = {}
        if start_date != None:
            filter_date['$gt'] = dt.strptime(start_date, '%Y-%m-%d')
        if end_date != None:
            filter_date['$lt'] = dt.strptime(end_date, '%Y-%m-%d')
        filter = {}
        if len(filter_date):
            filter['start_time'] = filter_date
        if len(exp_names):
