...
class AppLogic():
    ...
    def table_content_running(self, db_name):
        columns = ['_id', 'start_time', 'status', 'config.modif', 'config.gpu']
        cursor = self.data_manager.get_running_rows(db_name, columns)
        table_content = self.generate_experiment_table_rows(cursor, columns)
        return table_content

    def exp_infos(self, db_name, id):
        return self.data_manager.get_exp_infos(db_name, id)

    def table_content_from_exp_names(
        self,
        db_name,
        exp_names,
        columns,
        ...
        cursor = self.data_manager.get_rows_from_exp_names(
            db_name,
            exp_names,
            columns,
            start_date,
    ...
    def update_run_by_id(self, db_name, ids, status):
        self.data_manager.update_run_by_id(db_name, ids, status)

    def configs_from_ids(self, db_name, selected_ids):
        cursor = self.data_manager.get_configs_from_ids(db_name, selected_ids)
        res = []
        for elt in cursor:
            res.append(pp.pformat(elt['config'], indent=4))
    ...
    def generate_experiment_table_rows(self, cursor, columns):
        ...
        for exp in cursor:
            ...
            for col in columns:
                try:
                    item = self.get_nested(exp, col.split('.'))
                    item = pp.pformat(item, indent=4) if isinstance(item, dict) else item
                    row[col] = item
                except KeyError:
